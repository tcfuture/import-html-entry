 ...
 * 缓存子模块 exports
 */
const scriptExportsLifecycleCache = {};
/**
...
export function execScripts(entry, scripts, proxy = window, opts = {}) {
	...
	return getExternalScripts(scripts, fetch, error).then((scriptsText) => {
		...
		function exec(scriptSrc, inlineScript, resolve) {
			...
			if (scriptSrc === entry) {
				if (!scriptExportsLifecycleCache[scriptSrc]) {
					noteGlobalProps(strictGlobal ? proxy : window);

					try {
						// bind window.proxy to change `this` reference in script
						geval(
							getExecutableScript(scriptSrc, inlineScript, proxy, strictGlobal)
						);
						const exports =
							proxy[getGlobalProp(strictGlobal ? proxy : window)] || {};

						// 把子模块的 lifecycle 缓存起来
						scriptExportsLifecycleCache[scriptSrc] = exports;
					} catch (e) {
				...
				// 使用缓存的 lifecycle
				resolve(scriptExportsLifecycleCache[scriptSrc]);
			} else {
